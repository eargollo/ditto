...
func ListScanRoots(ctx context.Context, q Querier) ([]ScanRoot, error) {
	list, err := ListFolders(ctx, q)
	if err != nil {
		return nil, err
	}
...
func AddScanRoot(ctx context.Context, q Querier, path string) (int64, error) {
	return AddFolder(ctx, q, path)
}

// GetScanRoot returns the folder with the given id, or sql.ErrNoRows if not found.
func GetScanRoot(ctx context.Context, q Querier, id int64) (*ScanRoot, error) {
	f, err := GetFolder(ctx, q, id)
	if err != nil {
		return nil, err
	}
...
func DeleteScanRoot(ctx context.Context, q Querier, id int64) (bool, error) {
	return DeleteFolder(ctx, q, id)
}
