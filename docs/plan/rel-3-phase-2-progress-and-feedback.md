# Release 3, Phase 2: Progress and feedback (improvements)
...
**Context:** After Release 2, starting a scan redirects to a progress page that polls status. Today we only show high-level timestamps and final counts (e.g. hashed file count after hash phase completes). During the run there is no indication of progress or current activity, which can make the experience feel slow or unresponsive.

**References:** [ADR-004](../decisions/ADR-004-htmx-tailwind.md) (HTMX, Tailwind). Builds on Release 2 progress page and status fragment.
...
**Options:**
- **A)** Use existing data: while the scan is running, the walk is upserting files and inserting ledger rows; the status endpoint can report `files_discovered_so_far = COUNT(*) FROM file_scan WHERE scan_id = ?` (or equivalent). Polling the existing status endpoint already gives an increasing count until `completed_at` is set.
...
- **By inode (hardlinks):** As soon as the scan has upserted file rows and ledger, we have `inode` and `device_id`. Inode duplicate groups can be computed and shown as soon as the scan phase has completed (or even during scan, with the usual caveat that counts may still increase).
...
- [ ] All of the above work with the existing polling mechanism; no regression to Release 2 behavior.
